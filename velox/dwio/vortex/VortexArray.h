...
class VortexArray {
 public:
  ...
  static VortexArray fromOwned(const vx_velox_array* array);

  /// Returns the number of logical values in the array.
  size_t size() const;

  /// Returns an owned field array from a struct array.
  VortexArray field(const vx_velox_session* session, size_t index) const;
  ...
  const vx_velox_array* get() const;

  /// Returns true when this wrapper contains no array.
  explicit operator bool() const;

 private:
  explicit VortexArray(std::shared_ptr<const vx_velox_array> array);

  // Retains the immutable array through the Vortex reference count.
  std::shared_ptr<const vx_velox_array> array_;
};
